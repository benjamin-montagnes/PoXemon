...
Pokemon_Button::Pokemon_Button(std::string& name, int& lvl, int& h, int& i) {
    ...
    //Text
    if (!font.loadFromFile("Fonts/upheavtt.ttf")) { std::cout << "could not  pokemon button font" << std::endl; }
...
//    bar.setScale(sf::Vector2f(0.42, 0.4));
...
//    heart.setScale(sf::Vector2f(1.1, 1.1));
...
//    outline.setSize(sf::Vector2f(240, 30));

    //bar that shows the health
    healthbar.setSize(sf::Vector2f(health * healthwidth / 100, healthheight));
//    healthbar.setSize(sf::Vector2f(2*health * healthwidth / 100, 2*healthheight));
