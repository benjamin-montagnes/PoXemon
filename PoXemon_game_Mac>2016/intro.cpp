...
int menu(std::string picture) {
    sf::RenderWindow menu(sf::VideoMode(2800, 1400), "PoXemon", sf::Style::Titlebar | sf::Style::Close);
    sf::View view(sf::Vector2f(272, 272), sf::Vector2f(544, 272));
    view.zoom(0.6f);
    menu.setPosition(sf::Vector2i(50, 200));
    ...
    sf::Text title("Start Game", font, 90);
    title.setStyle(sf::Text::Bold);
    title.setFillColor(sf::Color::Black);
    title.setPosition(WindowSize.x*(0.43), WindowSize.y*(0.65));
    
    // Create an explanation
    sf::Text expl("Press X to go forward", font, 50);
...
int loadgame(std::string picture) {
    ...
    // Set the Icon
    sf::Image icon;
    if (!icon.loadFromFile("Sprites/icon.png")) std::cout << "Error" << std::endl;
    menu.setIcon(icon.getSize().x, icon.getSize().y, icon.getPixelsPtr());

    // Load a sprite to display
    sf::Texture texture;
    if (!texture.loadFromFile(/* resourcePath() + */ picture)) std::cout << "Error" << std::endl;
    sf::Sprite background;
    sf::Vector2u TextureSize;
    sf::Vector2u WindowSize;
    TextureSize = texture.getSize();
    ...
    // Create a graphical text to display
    sf::Font font;
    if (!font.loadFromFile(/* resourcePath() + */ "Fonts/sansation.ttf")) std::cout << "Error" << std::endl;
    sf::Text title("New Game", font, 80);
    title.setStyle(sf::Text::Bold);
    title.setFillColor(sf::Color::Black);
    title.setPosition(WindowSize.x*(0.28), WindowSize.y*(0.60));
    sf::Text title2("Load Game", font, 80);
    title2.setStyle(sf::Text::Bold);
    title2.setFillColor(sf::Color::Black);
    title2.setPosition(WindowSize.x*(0.60), WindowSize.y*(0.60));
    
    // Create an explanation
    sf::Text expl("or click on New Game", font, 50);
    expl.setStyle(sf::Text::Bold);
    expl.setFillColor(sf::Color::Black);
    expl.setPosition(WindowSize.x*(0.78), WindowSize.y*(0.90));
    
    sf::Text expl2("Paste the load file path then press Load Game", font, 50);
    expl2.setStyle(sf::Text::Bold);
    expl2.setFillColor(sf::Color::Black);
    expl2.setPosition(WindowSize.x*(0.58), WindowSize.y*(0.85));
    ...
    sf::RectangleShape choice1(sf::Vector2f(600.f, 100.f));
...
int intro(std::string picture, bool side, int which) {
    ...
    sf::RenderWindow intro(sf::VideoMode(2800, 1400), "PoXemon", sf::Style::Titlebar | sf::Style::Close);
    sf::View view(sf::Vector2f(272, 272), sf::Vector2f(544, 272));
    view.zoom(0.6f);
    intro.setPosition(sf::Vector2i(50, 200));
    ...
    // Set the Icon
    sf::Image icon;
    if (!icon.loadFromFile(/* resourcePath() +  */"Sprites/icon.png")) std::cout << "Error" << std::endl;
    intro.setIcon(icon.getSize().x, icon.getSize().y, icon.getPixelsPtr());

    // Load a background to display
    sf::Texture texture;
    if (!texture.loadFromFile(/* resourcePath() +  */"Sprites/menu_2.jpg")) std::cout << "Error" << std::endl;
    sf::Sprite background;
    sf::Vector2u TextureSize;
    sf::Vector2u WindowSize;
    TextureSize = texture.getSize();
    ...
    sf::Texture image;
    if (!image.loadFromFile(/* resourcePath() + */ picture)) std::cout << "Error" << std::endl;
    sf::Sprite professor;
    sf::Vector2u ImageSize;
    professor.scale(sf::Vector2f(2.f, 2.f)); //Set scale
    
    // Create a dialogue box
    sf::RectangleShape dialogue(sf::Vector2f(1700.f, 300.f));
    ...
    subtitle.setCharacterSize(80);
    subtitle.setStyle(sf::Text::Bold);
    subtitle.setFillColor(sf::Color::Black);
    
    if (side == 0){
    professor.setPosition(rt.left + 0.15*rt.width, rt.top + 1.6*rt.height);
    ...
    if (side == 1){
    professor.setPosition(rt.left + 5*rt.width, rt.top + 1.6*rt.height);
    professor.setTexture(image);
    ...
    // Create an explanation
    sf::Text expl("Press X to go forward", font, 60);
    expl.setStyle(sf::Text::Bold);
    expl.setFillColor(sf::Color::Black);
    if (side == 1) expl.setPosition(WindowSize.x*(0.10), WindowSize.y*(0.90));
    if (side == 0) expl.setPosition(WindowSize.x*(0.70), WindowSize.y*(0.90));
    ...
    if (which == 4){
        ...
        while (str_2.length() < 40) str_2 = " " + str_2;
...
bool choice(std::string prof, bool side, int which) {
    ...
    sf::RenderWindow choice(sf::VideoMode(2800, 1400), "PoXemon", sf::Style::Titlebar | sf::Style::Close);
    sf::View view(sf::Vector2f(272, 272), sf::Vector2f(544, 272));
    view.zoom(0.6f);
    choice.setPosition(sf::Vector2i(50, 200));
    ...
    // Create an image to appear
    sf::Texture image;
    if (!image.loadFromFile(/* resourcePath() + */ prof)) std::cout << "Error" << std::endl;
    sf::Sprite professor;
    sf::Vector2u ImageSize;
    professor.scale(sf::Vector2f(2.f, 2.f)); //Set scale
    ...
    // Create a choice 2
    sf::RectangleShape choice2(sf::Vector2f(600.f, 100.f));
    ...
    dialogue.setPosition(rt.left + 1*rt.width, rt.top - 0.5*rt.height);
    title.setPosition(rt.left + 1*rt.width, rt.top - 0.4*rt.height);
    subtitle.setPosition(rt.left + 1.3*rt.width, rt.top + 0.25*rt.height);
    ...
    dialogue.setPosition(rt.left + 0.10*rt.width, rt.top - 0.5*rt.height);
    title.setPosition(rt.left - 0.12*rt.width, rt.top - 0.4*rt.height);
    subtitle.setPosition(rt.left + 0.23*rt.width, rt.top + 0.25*rt.height);
    choice1.setPosition(rt.left + 0.2*rt.width, rt.top + 3*rt.height);
    choice2.setPosition(rt.left + 3.3*rt.width, rt.top + 3*rt.height);
    option1.setPosition(rt.left - 1.2*rt.width, rt.top + 3*rt.height);
    option2.setPosition(rt.left + 2*rt.width, rt.top + 3*rt.height);
    }
    
    // Create an explanation
    sf::Text expl("Press the left arrow for left choice, right arrow for the right choice", font, 50);
    expl.setStyle(sf::Text::Bold);
    expl.setFillColor(sf::Color::Black);
    if (side == 1) expl.setPosition(WindowSize.x*(0.05), WindowSize.y*(0.90));
    if (side == 0) expl.setPosition(WindowSize.x*(0.70), WindowSize.y*(0.90));
    ...
    title.setString(str);
    std::string str_2 = "Are you a boy or a girl?";
...
int choose(std::string prof, bool side) {
    ...
    sf::RenderWindow choose(sf::VideoMode(2800, 1400), "PoXemon", sf::Style::Titlebar | sf::Style::Close);
    sf::View view(sf::Vector2f(272, 272), sf::Vector2f(544, 272));
    view.zoom(0.6f);
    choose.setPosition(sf::Vector2i(50, 200));
    ...
    // Create a choice 3
    sf::RectangleShape choice3(sf::Vector2f(400.f, 100.f));
    ...
    professor.setPosition(rt.left + 5.2*rt.width, rt.top + 1.6*rt.height);
    professor.setTexture(image);
    dialogue.setPosition(rt.left - 0.2*rt.width, rt.top - 0.5*rt.height);
    title.setPosition(rt.left - 0.12*rt.width, rt.top - 0.4*rt.height);
    subtitle.setPosition(rt.left + 0.23*rt.width, rt.top + 0.2*rt.height);
    choice1.setPosition(rt.left + 0.10*rt.width, rt.top + 4*rt.height);
    choice2.setPosition(rt.left + 2*rt.width, rt.top + 4*rt.height);
    choice3.setPosition(rt.left + 4*rt.width, rt.top + 4*rt.height);
    option1.setPosition(rt.left - 0.5*rt.width, rt.top + 4.1*rt.height);
    option2.setPosition(rt.left + 1.4*rt.width, rt.top + 4.1*rt.height);
    option3.setPosition(rt.left + 3.4*rt.width, rt.top + 4.1*rt.height);
    
    // Create an explanation
    sf::Text expl("Press the left arrow for left choice, down arrow for the middle choice, right arrow for the right choice", font, 40);
    expl.setStyle(sf::Text::Bold);
    expl.setFillColor(sf::Color::Black);
    if (side == 1){
        expl.setPosition(WindowSize.x*(0.05), WindowSize.y*(0.90));
    }
    if (side == 0){
