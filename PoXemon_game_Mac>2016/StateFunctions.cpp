...
//    start_button.setSize(sf::Vector2f(2*x/5,2*y/5));
//    info_button.setSize(sf::Vector2f(2*x/5,2*y/5));
...
//    opp.setScale(sf::Vector2f(1,1));
    star.setScale(sf::Vector2f(1.50, 1.50)); // How to make it relate to window size???
    opp.setScale(sf::Vector2f(2,2));
    opp.setOrigin(0,0);
...
// tout ce qui a au dessus n'influence pas les attacques avec pokemon dans tall grass ni dans water
...
//    intro_text.setCharacterSize(50);
    choose_pokemon.setCharacterSize(40);
    intro_text.setCharacterSize(100);
...
//    opp_name.setCharacterSize(30);
    opp_name.setCharacterSize(60);
//    opp_name.setPosition(0, 0);
    opp_name.setPosition(window.getSize().x / 2 + 3 * window.getSize().x / 25 - 50, window.getSize().y / 2 + window.getSize().y / 15 - (80 + star.getGlobalBounds().height / 2));
    //opponent level
    opp_lvl.setString("");
...
//                run_sprite.setScale(1.2, 1.2); // slightly bigger when the mouse is on the button
                run_sprite.setScale(2.4, 2.4); // slightly bigger when the mouse is on the button
...
//    text_fainted.setCharacterSize(50);
    choose_pokemon.setCharacterSize(60);
    text_fainted.setCharacterSize(100);
    choose_pokemon.setFillColor(sf::Color::Black);
...
//        leave_fight.setCharacterSize(30);
        leave_fight.setCharacterSize(60);
...
//                running_sprite.setScale(2.2, 2.2); // slightly bigger when the mouse is on the button
                running_sprite.setScale(4.4, 4.4); // slightly bigger when the mouse is on the button
                if (sf::Mouse::isButtonPressed(sf::Mouse::Left)) {
                    clicked_button = -1;
                }
            }
...
//    lost_fight.setCharacterSize(60);
    lost_fight.setCharacterSize(120);
