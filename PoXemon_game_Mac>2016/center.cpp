...
//    pokemon_bag = {"jistolwer","auron","husabus"};
    ...
    if (!font.loadFromFile(/* resourcePath() + */ "Fonts/sansation.ttf")) std::cout<<"Problem"<<std::endl;
...
void Center::heal_pokemons(BackpackMap &backpack, sf::Event &event, Trainer &trainer) {
    if (event.type == sf::Event::KeyPressed&&event.key.code == sf::Keyboard::X&&Selecteditem == 0){
        ...
        trainer.state = "healed";
        
    }
    if (event.type == sf::Event::KeyPressed&&event.key.code == sf::Keyboard::W&&Selecteditem == 0){
        trainer.state = "Stop";
    }
}
